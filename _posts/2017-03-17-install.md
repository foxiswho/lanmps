...
LANMPS 一键安装包V3.3.1 ：Linux+Nginx+Mysql+PHP+Elasticsearch ( phpmyadmin+opencache+xdebug )套件包,绿色PHP套件，一键自动安装。
> 
...
### 方法二：
下载地址：http://download.lanmps.com/lanmps/lanmps-3.3.1.tar.gz (七牛免费资源,每月10G流量)
...
`tar -zxvf lanmps-3.3.1.tar.gz && cd lanmps-3.3.1 && ./lanmps.sh`
